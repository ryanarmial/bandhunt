      ...
      <div class="col-md-6">
        <div class="form-group" style="margin-top:25px;margin-bottom:10px">
          <select class="form-control" onchange="location = this.value;">
            <option selected value="{{url('levis-tools/leaderboard')}}">Semua Tanggal</option>
            @foreach($dates as $date)
              @if($date->tanggal == $tanggal)
              <option selected value="{{url('levis-tools/leaderboard')}}/{{$date->tanggal}}">{{$date->tanggal}}</option>
              @else
              <option value="{{url('levis-tools/leaderboard')}}/{{$date->tanggal}}">{{$date->tanggal}}</option>
              @endif
